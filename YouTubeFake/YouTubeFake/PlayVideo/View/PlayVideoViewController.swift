...
class PlayVideoViewController: BaseViewController {
    ...
    lazy var controller = PlayVideoController(delegate: self)
    ...
    private func loadDataFromApi(){
        Task{ [weak self] in
            await self?.controller.getVideos(videoId)
        }
    }
    ...
    private func configTableView(){
        ...
        //Register TableView
        tableView.register(cell: VideoHeaderTableViewCell.self)
        tableView.register(cell: VideoFullTableViewCell.self)
...
extension PlayVideoViewController: UITableViewDelegate, UITableViewDataSource {
    func numberOfSections(in tableView: UITableView) -> Int {
        return controller.relatedVideoList.count
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return controller.relatedVideoList[section].count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let item = controller.relatedVideoList[indexPath.section]
        if indexPath.section == 0{
            guard let video = item[indexPath.row] as? ItemVideo else{ return UITableViewCell()}
            
            let videoHeaderCell = tableView.dequeueReusableCell(for: VideoHeaderTableViewCell.self, for: indexPath)
            videoHeaderCell.configCell(videoModel: video, channelModel: controller.channelModel)
            ...
            guard let video = item[indexPath.row] as? ItemVideo else{ return UITableViewCell()}
            let videoFullWidthCell = tableView.dequeueReusableCell(for: VideoFullTableViewCell.self, for: indexPath)
...
extension PlayVideoViewController: PlayVideoViewProtocol {
    func getRelatedVideosFinished() {
        if let video = controller.relatedVideoList[0].first as? ItemVideo,
           let title = video.snippet?.title{
            videoTitleLabel.text = title
        }
        channelTitleLabel.text = controller.channelModel?.snippet.title
